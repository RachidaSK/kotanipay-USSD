...
const mpesaApi = new Mpesa({ 
  consumerKey: functions.config().mpesa.consumer.key,
  consumerSecret: functions.config().mpesa.consumer.secret,
  environment: functions.config().mpesa.env,
  shortCode: functions.config().mpesa.shortcode,
  initiatorName: functions.config().mpesa.initiator_name,
  lipaNaMpesaShortCode: functions.config().mpesa.lnm.shortcode,
  lipaNaMpesaShortPass: functions.config().mpesa.lnm.shortpass,
  securityCredential: functions.config().mpesa.security_creds
});
...
const iv = functions.config().crypto_iv.key;
const enc_decr_fn = functions.config().algo.enc_decr;
const  phone_hash_fn = functions.config().algo.msisdn_hash;

// AFRICASTALKING API
const AT_credentials = {
    apiKey: functions.config().at_api.key,
    username: functions.config().at_api.usename
}
...
const NODE_URL = functions.config().celo.node_url; 
...
mpesaApp.post("/lipanampesa/success", async (req, res) => {
  ...
  // axios.post(url, callbackdata).then(response => {console.log('Response: ',response.status)})
  let depositMSISDN = `${callbackdata[4].Value}`;  // phoneNumber to send sms notifications

  const escrowMSISDN = functions.config().celo.escrow_msisdn;
...
async function signupDeposit(publicAddress){
  let escrowMSISDN = functions.config().escrow.msisdn;
  let amount = 10;
...
function getEncryptKey(userMSISDN){    
  const crypto = require('crypto');
  const hash_fn = functions.config().algo.key_hash;
...
function createNewUser(userId, userMSISDN){
  return new Promise(resolve => {
      admin.auth().createUser({ uid: userId, phoneNumber: `+${userMSISDN}` })
      .then(function(userRecord) { console.log('Successfully created new user:', userRecord.uid); })
      .catch(function(error) { console.log('Error creating new user:', error); });
  });  
}
...
exports.kotanipay = functions.https.onRequest(app); 
